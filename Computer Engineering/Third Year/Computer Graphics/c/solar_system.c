// Problem Statement: 
// Write a program to show animation (Object should be 3D)
// Solar System 

// Algorithm/ Pseudocode for each process: 

// 1.	We first defined the orbits of the planets. Then we tilted them to by 45 and 63 degrees in the z-plane to get the 3d effect. 
// 2.	We then define the planets and the sun 
// 3.	We also define the lighting of the sun and the light that it gives out to the other planets.
// 4.	After that, we define the asteroid belt with 300 small asteroids and 4 tracks. 
// 5.	Also, we then define the motion, at each timestamp we make the planets translate as well as rotate by a specific angle. It gives us the motional effect. 
// 6.	We also defined the moon of the Earth in a similar fashion and made it translate using the same logic. 
// 7.	Also, we enlarge and make the sun smaller after a certain timestamp so that it helps us identify the object that is illuminating all the other objects. 
