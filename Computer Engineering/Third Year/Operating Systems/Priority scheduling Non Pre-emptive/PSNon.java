  ...
  public static void main(String[] args) {
    ...
    for(int i = 0; i < n-1; i++)
    {
      for (int j = 0; j < n-i-1; j++) 
          if (process_priority[j] > process_priority[j+1]) 
          { 
              // swap arr[j+1] and arr[i] 
              int temp = process_priority[j];
              process_priority[j] = process_priority[j+1]; 
              process_priority[j+1] = temp; 

              int temp3 = process_burst_time[j];
              process_burst_time[j] = process_burst_time[j+1]; 
              process_burst_time[j+1] = temp3;
